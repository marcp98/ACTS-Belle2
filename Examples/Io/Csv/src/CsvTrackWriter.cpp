...
ProcessCode CsvTrackWriter::writeT(
    const AlgorithmContext& context, const ConstTrackContainer& tracks) {
	// open per-event file
	std::string path = perEventFilepath(m_cfg.outputDir, m_cfg.fileName, context.eventNumber);
	...
	for (const auto& track : tracks) {
		...
		if (!track.hasReferenceSurface()) {
			ACTS_WARNING("No fitted track parameters for trajectory with entry index = "
			    << track.tipIndex());
			continue;
		}
		...
		if (m_cfg.onlyTruthMatched && particleHitCount.empty()) {
			ACTS_WARNING("No truth particle associated with this trajectory with entry "
					 "index = "
			    << track.tipIndex());
			continue;
		}
