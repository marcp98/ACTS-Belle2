...
ActsExamples::CsvMyTrackParameterWriter::CsvMyTrackParameterWriter(
    const ActsExamples::CsvMyTrackParameterWriter::Config& config,
    Acts::Logging::Level level)
    : m_cfg(config),
      m_logger(Acts::getDefaultLogger("CsvMyTrackParameterWriter", level)) {
...
ActsExamples::CsvMyTrackParameterWriter::~CsvMyTrackParameterWriter() = default;

std::string ActsExamples::CsvMyTrackParameterWriter::name() const {
  return "CsvMyTrackParameterWriter";
}

ActsExamples::ProcessCode ActsExamples::CsvMyTrackParameterWriter::finalize() {
  // Write the tree
  return ProcessCode::SUCCESS;
}

ActsExamples::ProcessCode ActsExamples::CsvMyTrackParameterWriter::write(
  ...
  BoostDescribeCsvWriter<MyTrackParameterData> writer(
      path, m_cfg.outputPrecision);

  MyTrackParameterData data{};
  for (const auto& track : inputTracks) {
    ...
    data.protoId = track.component<std::size_t>("protoId");
