  ...
  if (numcLayers != 0u) {
    ...
    for (std::size_t icl = 0; icl < numcLayers; ++icl) {
      ...
      for (auto& moduleCenter : m_cfg.centralModulePositions.at(icl)) {
        ...
        if ((icl==1 && moduleCenter.z() > 100 ) || (icl==2 && moduleCenter.z() > 140)) {
          double slanted_angle = -0.207; // in rad, corresponds to 11.87 degree
          // the local z axis is the normal vector
          Acts::Vector3 moduleLocalZ(std::cos(modulePhi + modulePhiTilt)*std::cos(slanted_angle),
                                   std::sin(modulePhi + modulePhiTilt)*std::cos(slanted_angle), -std::sin(slanted_angle));
          // the local y axis is the global z axis
          Acts::Vector3 moduleLocalY(std::cos(modulePhi + modulePhiTilt) * std::sin(slanted_angle),
                                     std::sin(modulePhi + modulePhiTilt) * std::sin(slanted_angle),
                                     std::cos(slanted_angle));
        ...
        if (!m_cfg.centralModuleBacksideGap.empty()) {
          ...
          Acts::Vector3 moduleLocalZ = moduleRotation.col(2); 
...
std::shared_ptr<ActsPlugins::TGeoDetectorElement> ProtoLayerCreator::defaultElementFactory(
    ...
    // Einfach weiterleiten, damit es gültig ist
