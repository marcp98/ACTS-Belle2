	...
	namespace {
		...
		class Gen3Belle2Builder : public Belle2Builder {
		  public:
			...
			void buildSVD(
			    Acts::Experimental::BlueprintNode& parent, const Acts::GeometryContext& gctx);

			std::vector<ProtoLayerSurfaces> buildLayers(const Acts::GeometryContext& gctx);
			...
			static std::shared_ptr<const Acts::HomogeneousSurfaceMaterial> asHomogeneous(
				...
				if (hm == nullptr) {
					throw std::runtime_error(
					    std::string { debugLabel } + " material is not homogeneous");
				}
				return hm;
			}
			...
			std::unique_ptr<Acts::NavigationPolicyFactory> createNavigationPolicy2() const {
				return Acts::NavigationPolicyFactory {}
		...
		Gen3Belle2Builder::buildTrackingGeometry(const Acts::GeometryContext& gctx,
		    std::optional<std::filesystem::path> graphvizFile) {
			...
			auto trackingGeometry = root.construct(options, gctx, *logger().clone("Blprnt"));
			ACTS_INFO("Lade material_map.json direkt in C++...");

			return trackingGeometry;
		}

		void Gen3Belle2Builder::buildSVD(
		    Acts::Experimental::BlueprintNode& parent, const Acts::GeometryContext& gctx) {
			using namespace Acts::Experimental;
			using namespace Acts::UnitLiterals;
			using enum Acts::CylinderVolumeBounds::Face;
			...
			std::vector<double> z_ranges = {140.0_mm, 190.0_mm, 275.0_mm, 325.0_mm};

			auto svdBounds
			    = std::make_shared<Acts::CylinderVolumeBounds>(0_mm, 180_mm, 380_mm);
			...
			std::vector<Acts::Transform3> z_transformations = {transformationZL3, transformationZL4, transformationZL5, transformationZL6};
			struct TrackingVolumeAccessor : public Acts::Experimental::StaticBlueprintNode {
			...
			parent.addMaterial("SVD_PassiveMaterial", [&](auto& matNode) {
				...
				// Outer cylinder portal: binned in phi x z
				...
				// Negative endcap disc portal: binned in r x phi
				...
				// Positive endcap disc portal
				matNode.configureFace(Face::PositiveDisc,
				    Acts::DirectedProtoAxis(AxisR, Open, 0_mm, 180_mm, 8),
				    Acts::DirectedProtoAxis(AxisPhi, Closed, -M_PI, M_PI, 8));

				matNode.addStaticVolume(Acts::Transform3::Identity(), svdBounds, "SVD_Container", [&](auto& svdVol) {
					for (int i = 0; i < 4; ++i) {
						std::string name = "SVD_Layer" + std::to_string(i + 3);

						// 1. Der AKTIVE Layer (Deine Sensoren)
						svdVol.addLayer(name, [&](auto& layer) {
							...
							createNavigationPolicy3(LayerType::Cylinder,
						...
						svdVol.addLayer(suppName, [&](auto& suppLayer) {
							...
							double layerRadius = svdLayers[i].protoLayer.medium(Acts::AxisDirection::AxisR) - 2_mm;
							
							auto cylBounds = std::make_shared<Acts::CylinderBounds>(layerRadius, z_ranges[i]);
							auto cylSurf = Acts::Surface::makeShared<Acts::CylinderSurface>(z_transformations[i], cylBounds);

							Acts::BinUtility bu(120, -M_PI, M_PI, Acts::open, Acts::AxisDirection::AxisPhi);
							bu += Acts::BinUtility(100, -z_ranges[i], z_ranges[i], Acts::open, Acts::AxisDirection::AxisZ);
							cylSurf->assignSurfaceMaterial(std::make_shared<Acts::ProtoSurfaceMaterial>(bu));

							std::vector<std::shared_ptr<Acts::Surface>> passiveSurfaces = { cylSurf };
							Acts::MutableProtoLayer pl(gctx, passiveSurfaces);
							
							suppLayer.setProtoLayer(pl);
							suppLayer.setEnvelope(Acts::ExtentEnvelope {
								{ .z = { 2_mm, 2_mm },
									.r = { 2_mm, 2_mm } } });
		...
		std::vector<ProtoLayerSurfaces> Gen3Belle2Builder::buildLayers(
			...
			auto fillLayer = [&](const std::vector<std::shared_ptr<Acts::Surface>>& sVector,
				...
				pl.envelope[Acts::AxisDirection::AxisR]
				    = { lCfg.envelope.first, lCfg.envelope.second };
				pl.envelope[Acts::AxisDirection::AxisZ]
				    = { lCfg.envelope.second, lCfg.envelope.second };
			...
			for (const auto& layerCfg : layerConfigs) {
				...
				if (!layerCfg.splitConfigs.empty()) {
					for (const auto& [axisDir, tConfig] : layerCfg.splitConfigs) {
						ACTS_DEBUG("- layer splitting attempt in "
						    << axisDirectionName(axisDir) << " with tolerance "
						    << tConfig << ".");
					}
				}
				...
				if (tVolume == nullptr) {
					...
					if (node == nullptr) {
						...
						throw std::invalid_argument(
						    "Could not locate node for " + volname);
					}
				...
				if (tVolume != nullptr) {
					...
					ACTS_DEBUG("- applying  " << layerCfg.parseRanges.size()
									  << " search restrictions.");
					for (const auto& [axisDir, pRange] : layerCfg.parseRanges) {
						ACTS_VERBOSE(" - range " << axisDirectionName(axisDir)
										 << " within [ " << pRange.first << ", "
										 << pRange.second << "]");
					}
					...
					for (auto& snode : tgpState.selectedNodes) {
						...
						// 1. Erstelle die Vorderseite (Frontside)
						auto tgElementFront = m_cfg.detectorElementFactory(identifier,
						    *snode.node, *snode.transform, layerCfg.localAxes,
						...
						// 2. Erstelle die Rückseite (Backside), falls es ein DSSD ist
						if (layerCfg.isDSSD) {
							// TGeo nutzt intern Zentimeter! Daher Umrechnung für die
							// Translation
							double gapCm = layerCfg.backsideGap
							    / (1 * Acts::UnitConstants::cm);
							TGeoTranslation localShift(0, 0, gapCm);

							// TGeo Rotation nutzt Grad! Daher Umrechnung von rad in
							// deg
							//TGeoRotation localRot;
							//localRot.RotateZ(layerCfg.backsideStereo * 180.0 / M_PI);

							// Kombiniere Translation und Rotation
							//TGeoCombiTrans localTrans(localShift, localRot);

							// Multipliziere lokale Transformation auf die globale
							// TGeoMatrix
							TGeoHMatrix backTransform(*snode.transform);
							backTransform.Multiply(&localShift);

							// Eigener Identifier für die Rückseite (wir setzen
							// einfach das niedrigste Bit auf 1) Da in
							// MyIdentifierProvider das Sensor-Feld um 5 geshiftet
							// wird, ist Bit 0 frei.
							...
							sVector.push_back(
							    tgElementBack->surface().getSharedPtr());
						}
					}
...
Belle2::Belle2(const Config& cfg)
    : Detector(Acts::getDefaultLogger("Belle2", cfg.logLevel))
    , m_cfg(cfg) {
	...
	// Auch hier wurde Schabernack betrieben, diesmal mit der detectorelement factory
	...
	B2::Belle2Builder::LayerConfig cl3 { .volumeName = "SVD.Envelope",
	...
	B2::Belle2Builder::LayerConfig cl4 { .volumeName = "SVD.Envelope",
		...
		.envelope = { 5, 5 },
		.localAxes = ActsPlugins::TGeoAxes("XYZ")};
	B2::Belle2Builder::LayerConfig cl5 { .volumeName = "SVD.Envelope",
		.sensorNames = { "SVD.5.*.2.Active", "SVD.5.*.3.Active", "SVD.5.*.4.Active" },
		...
		.envelope = { 5, 5 },
		.localAxes = ActsPlugins::TGeoAxes("XYZ") };
	B2::Belle2Builder::LayerConfig cl6 { .volumeName = "SVD.Envelope",
		.sensorNames
		= { "SVD.6.*.2.Active", "SVD.6.*.3.Active", "SVD.6.*.4.Active", "SVD.6.*.5.Active" },
		...
		.envelope = { 5, 5 },
		.localAxes = ActsPlugins::TGeoAxes("XYZ") };
	B2::Belle2Builder::LayerConfig cl4s { .volumeName = "SVD.Envelope",
		...
		.envelope = { 5, 5 },
		.localAxes = ActsPlugins::TGeoAxes("XYZ")};
	B2::Belle2Builder::LayerConfig cl5s { .volumeName = "SVD.Envelope",
		...
		.envelope = { 5, 5 },
		.localAxes = ActsPlugins::TGeoAxes("XYZ")};
	B2::Belle2Builder::LayerConfig cl6s { .volumeName = "SVD.Envelope",
	...
	Acts::MaterialSlab svdSlab(B2::Belle2Builder::kSiliconMaterial,
	    0.32f); // 320 micron SVD sensor thickness
	auto svdModuleMaterial = m_cfg.buildProto
	...
	// Then assign to each layer config:
	...
	cl3.backsideStereo = M_PI / 2.0; // 90 Grad verdreht für orthogonale Strips
	...
	m_trackingGeometry
	    = builder.buildTrackingGeometry(m_nominalGeometryContext, m_cfg.graphvizFile);
	...
	if (m_cfg.materialDecorator) {
        ...
        m_trackingGeometry->visitSurfaces([&](const Acts::Surface* surface) {
            if (surface) {
                // Wir müssen const_cast nutzen, da decorate() die Fläche verändert
		...
		m_trackingGeometry->visitVolumes([&](const Acts::TrackingVolume* volume) {
            if (volume) {
                // Das Volumen selbst dekorieren
                m_cfg.materialDecorator->decorate(const_cast<Acts::TrackingVolume&>(*volume));
                
                // Alle Hüllen (Portale) dieses Volumens dekorieren
                for (const auto& boundary : volume->boundarySurfaces()) {
                    if (boundary) {
                        const Acts::Surface& portalSurface = boundary->surfaceRepresentation();
                        m_cfg.materialDecorator->decorate(const_cast<Acts::Surface&>(portalSurface));
                    }
                }
            }
        });
	...
	m_detectorStore.insert(m_detectorStore.end(), elements.begin(), elements.end());
